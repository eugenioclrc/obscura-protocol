...
contract NoirHelper is TestBase {
    ...
    function useName(string memory circuitName) internal view returns (string memory, string memory, string memory) {
        string memory rand = vm.toString(vm.randomUint(32));
        return (string.concat("Prover", rand), string.concat("proof", rand), string.concat(circuitName, rand));
    }
        ...
        returns (bytes32[] memory, bytes memory)
    {
        ...
        assembly {
            mcopy(add(proof, 0x20), add(add(proofData, _inputSize), 0x20), sub(mload(proofData), _inputSize))
        }
    ...
    function readProofHonk(uint256 pubInputSize) public view returns (bytes32[] memory, bytes memory) {
        ...
        returns (bytes32[] memory, bytes memory)
    {
        ...
        for (uint256 i = 0; i < pubInputSize; i++) {
            assembly {
                let index := mul(0x20, i)
                mstore(add(0x20, add(index, publicInputs)), mload(add(0x24, add(add(0x60, index), proofData))))
            }
        }
        assembly {
            ...
            mcopy(add(proof, 0x80), add(0x84, add(proofData, _inputSize)), sub(mload(proofData), add(_inputSize, 0x84)))
        }
    ...
    /// ```
    function _generateProof(string memory proverName, uint256 pubInputSize, PlonkFlavour flavour, bool cleanup)
        internal
        returns (bytes32[] memory, bytes memory)
    {
        ...
        string memory circuitName =
            string(vm.parseTomlString(vm.readFile(string.concat(circuitProjectPath, "/Nargo.toml")), ".package.name"));
        ...
        for (uint256 i; i < inputs.structInputs.length; i++) {
            vm.writeToml(inputs.structInputs[i].structValues, proverTOML, inputs.structInputs[i].name);
    ...
    function generateProofHonk(uint256 pubInputSize) public returns (bytes32[] memory, bytes memory) {
